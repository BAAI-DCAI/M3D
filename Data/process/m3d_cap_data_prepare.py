...
def process_subfolder(subfolder):
    ...
    num_output_it = len(os.listdir(output_id_folder))
    num_input_it = len(os.listdir(input_id_folder))

    if num_input_it != num_output_it:
        ...
        for subsubfolder in os.listdir(input_id_folder):
            ...
            if os.path.isdir(subsubfolder_path):
                subsubfolder = remove_diacritics(subsubfolder)
...
# Process each subfolder using multithreading
with concurrent.futures.ThreadPoolExecutor(max_workers=32) as executor:
    list(tqdm(executor.map(process_subfolder, subfolders), total=len(subfolders), desc="Processing"))